...
| GitHub | Cloud bearer token with provider-verified numeric-user attestation | authenticated-user repository list, repository read, PR search/read, commit check runs | — | unavailable | principal check before apply; independent normalized re-read | disabled |
