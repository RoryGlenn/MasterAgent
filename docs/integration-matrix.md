...
|---|---|---|---|---|---|---|
| Jira | Cloud basic/API token at exact tenant root or `api.atlassian.com/ex/jira/{cloudId}`; Data Center bearer/basic | server info, search, issue read, exact allowlisted review context | update/comment/transition proposal | comment creation; update/transition disabled pending CAS | independent exact issue/relation read; manual comment deletion recovery | disabled |
...
| Reddit | Cloud delegated OAuth with separate read (`identity read history privatemessages`) and communication (`identity read submit`) refresh credentials plus provider-verified user-ID/scope attestation | search, content, subreddit rules, user history, inbox | post/comment/reply Markdown | post and comment/reply; edit/delete adapters catalog-disabled pending provider CAS; granular flags default false | reject missing/out-of-profile scopes; zero-retry mutations; exact re-read; quarantined edit/delete ownership/version tests; manual recovery | read profile available; effects disabled |
