...
route after its specification archival.

## Results

| Measure | Manual index baseline | Generated router | Result |
| --- | ---: | ---: | --- |
| Checked-in router bytes | 29,903 | 15,027 | 49.7% smaller |
| Approximate context tokens (`bytes / 4`) | 7,476 | 3,757 | 49.7% smaller |
| Production-module coverage | 80/97 direct links | 99/99 exact owners | Complete and machine-checked |
| Test-module coverage | 68/80 direct links | 83/83 exact owners | Complete and machine-checked |
| Current-requirement coverage | 0/16 direct links | 19/19 exact owners | Complete and machine-checked |
| Stable machine route IDs | 0 | 22 | Every declared route is addressable |
| Automated routing fixtures | 0/23 | 23/23 | 100% deterministic fixture accuracy |
| Median lookup time | 652.75 microseconds | 115.58 microseconds | 5.65 times faster |
| Example selected-route payload | Not available | 1,161 bytes | One route and its local agent contract |
...
assets and issue #110 added progressive operating modes. The baseline `0/23`
...
The metrics command parses the bounded TOML manifest, verifies all 23 routing
...
The manifest exactly owns 99 production Python modules, 83 test modules, 19
