...
| --- | ---: | ---: | --- |
| Checked-in router bytes | 29,903 | 14,237 | 52.4% smaller |
| Approximate context tokens (`bytes / 4`) | 7,476 | 3,560 | 52.4% smaller |
...
| Automated routing fixtures | 0/21 | 21/21 | 100% deterministic fixture accuracy |
| Median lookup time | 652.75 microseconds | 108.97 microseconds | 5.99 times faster |
| Example selected-route payload | Not available | 988 bytes | One route and its local agent contract |
...
assets. The baseline `0/21` records that the prose index did not expose a
...
the same 21 manifest routing fixtures without a stable route-ID interface. The
...
The metrics command parses the bounded TOML manifest, verifies all 21 routing
...
contract; it does not emit sibling profiles or the complete manifest.

## Coverage

At introduction, the manifest exactly owns 98 production Python modules, 81
test modules, 17 current requirements, 13 configurations, 31 CLI commands, 82
