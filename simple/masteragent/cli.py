...
def parser() -> argparse.ArgumentParser:
    ...
    root = argparse.ArgumentParser(
        prog="masteragent",
        description="Native work tools and remembered tasks for your assistant.",
    )
    root.add_argument("--version", action="version", version=__version__)
    root.add_argument(
        "--home",
        help="Local state directory (default MASTERAGENT_HOME or ~/.masteragent)",
    )
    root.add_argument(
        "--json",
        action="store_true",
        help="Return structured results for a host assistant",
    )
    commands = root.add_subparsers(dest="command", required=True)
    setup = commands.add_parser(
        "setup", help="Remember connection details and project defaults"
    )
    setup.add_argument("--provider", choices=PROVIDERS)
    setup.add_argument(
        "--url", help="Provider HTTPS base URL, including any server context path"
    )
    setup.add_argument("--deployment", choices=("cloud", "server"))
    setup.add_argument(
        "--token-env", help="Environment variable name containing the token"
    )
    setup.add_argument(
        "--username-env",
        help="Environment variable name containing the Cloud account email/username",
    )
    ...
    setup.add_argument(
        "--page",
        action="append",
        help="Default Confluence page, repeat for multiple pages",
    )
    setup.add_argument(
        "--check-json",
        action="append",
        help='Check command argv as JSON, e.g. ["python","-m","unittest","discover"]',
    )
    commands.add_parser(
        "doctor",
        help="Show local setup and missing environment names; no network calls",
    )
    commands.add_parser("context", help="Show the editable preferences file")
    review = commands.add_parser(
        "review", help="Collect issue, linked PR/builds, and documentation"
    )
    ...
    develop = commands.add_parser(
        "develop", help="Prepare an isolated worktree and handoff for host coding"
    )
    develop.add_argument("issue")
    develop.add_argument(
        "--base", help="Local Git base ref (default current repository HEAD)"
    )
    for name, help_text in (
        ("checks", "Run configured checks on a development worktree"),
        ("show", "Show a saved task"),
        ("resume", "Continue unfinished steps"),
        ("cancel", "Cancel a task before its next tool step"),
    ):
        command = commands.add_parser(name, help=help_text)
        command.add_argument("task")
    publish = commands.add_parser(
        "publish", help="Push tested commits, open a Bitbucket PR, and comment on Jira"
    )
    ...
    status = commands.add_parser(
        "status", help="Create a local status draft from saved task history"
    )
    ...
    resolve = commands.add_parser(
        "resolve", help="Record an observed result for an uncertain external write"
    )
    resolve.add_argument("task")
    resolve.add_argument("step")
    resolution = resolve.add_mutually_exclusive_group(required=True)
    resolution.add_argument(
        "--result-file", type=Path, help="JSON result observed in the provider"
    )
    resolution.add_argument(
        "--not-applied",
        action="store_true",
        help="I checked the provider and confirmed this write did not happen; allow retry",
    )
    commands.add_parser(
        "demo", help="Run fictional review and status workflows without credentials"
    )
    return root


def _setup(
    args: argparse.Namespace, home: Path, config: dict[str, Any]
) -> dict[str, Any]:
    if args.provider:
        if not args.url:
        ...
        (args.url, args.deployment, args.token_env, args.username_env, args.ca_bundle)
    ):
    ...
    if args.project:
        ...
        if args.check_json:
            ...
            if any(not isinstance(command, list) for command in checks):
                raise ValueError(
                    "Each --check-json value must be a JSON array of arguments."
                )
        configure_project(
            config,
            args.project,
    ...
    return {
        ...
        "context": str(home / "context.md"),
        "readiness": readiness(config),
        "next_action": "Select MasterAgent Simple in Copilot, or configure a provider with setup --provider NAME --url URL.",
    }
...
def execute(args: argparse.Namespace) -> Any:
    ...
    if args.command == "context":
        return {
            "path": str(home / "context.md"),
            "content": (home / "context.md").read_text(encoding="utf-8"),
        }
    with TaskStore(home / "tasks.sqlite3") as store:
        ...
        if args.command == "resolve":
            if args.not_applied:
                ...
                required = {
                    "git.push": ("branch", "commit", "remote"),
                    "bitbucket.create_pr": ("url",),
                    "jira.comment": ("id", "url"),
                }.get(args.step)
                if (
                    required is None
                    or not isinstance(result, dict)
                    or any(not result.get(field) for field in required)
                ):
                    raise ValueError(
                        f"Provide an observed result object containing {', '.join(required or ())} for a known publication step."
                    )
...
def _display(result: Any) -> None:
    if isinstance(result, list):
        ...
        for task in result:
            print(
                f"{task['id']}  {task['status']:10}  {task['workflow']}  {task['inputs'].get('issue', '')}"
            )
        ...
        if data.get("pull_request"):
            ...
            print(
                f"Draft: {'yes' if draft is True else 'no' if draft is False else 'not reported by provider'}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "Interrupted. Completed steps are saved; use tasks and resume to continue.",
            file=sys.stderr,
        )
        return 130
    if args.json:
