...
def config(cloud: bool = True) -> dict:
    ...
    return {
        "jira": {
            ...
            if cloud
            else "https://work.example/jira",
            "deployment": deployment,
        },
        "confluence": {
            "url": "https://team.atlassian.net/wiki"
            if cloud
            else "https://work.example/confluence",
            "deployment": deployment,
        },
        "bitbucket": {
            "url": "https://bitbucket.org"
            if cloud
            else "https://work.example/bitbucket",
            "deployment": deployment,
        },
    }
...
class ProviderTests(unittest.TestCase):
    def test_cloud_issue_extracts_adf_and_remote_links(self) -> None:
        fake = FakeTransport(
            [
                                            ...
                                            "marks": [
                                                {
                                                    "type": "link",
                                                    "attrs": {
                                                        "href": "https://bitbucket.org/team/repo/pull-requests/7"
                                                    },
                                                }
                                            ],
                                        }
                ...
                [
                    {
                        "object": {
                            "url": "https://team.atlassian.net/wiki/spaces/DEV/pages/9/Cache"
                        }
                    }
                ],
            ]
        ...
        self.assertEqual(
            fake.calls[0][1],
            "rest/api/3/issue/APP-12?fields=summary,description,status,issuelinks",
        )
    ...
    def test_server_issue_keeps_context_and_plain_text(self) -> None:
            ...
            [
                {
                    "fields": {
                        "summary": "Cache",
                        "description": "See https://work.example/bitbucket/projects/TEAM/repos/repo/pull-requests/8.",
                    }
                },
                [],
            ]
        ...
        self.assertEqual(
            result["links"],
            ["https://work.example/bitbucket/projects/TEAM/repos/repo/pull-requests/8"],
        )
    ...
    def test_cloud_pr_and_paginated_builds(self) -> None:
        ...
        fake = FakeTransport(
            [
                ...
                {"values": [{"name": "Lint", "state": "FAILED"}]},
            ]
        )
        providers = Providers(config(), fake)
        pr = providers.pull_request(
            "https://bitbucket.org/team/repo/pull-requests/7/diff#part"
        )
    ...
    def test_server_pr_and_builds_use_context_api(self) -> None:
        ...
        pr = providers.pull_request(
            "https://work.example/bitbucket/projects/TEAM/repos/repo/pull-requests/8/overview"
        )
        self.assertEqual(pr["target_branch"], "main")
        self.assertEqual(providers.builds(pr)[0]["name"], "build")
        self.assertEqual(
            fake.calls[0][1], "rest/api/1.0/projects/TEAM/repos/repo/pull-requests/8"
        )
        self.assertEqual(
            fake.calls[1][1],
            "rest/api/1.0/projects/TEAM/repos/repo/commits/abcdef1234/builds?limit=100",
        )
    ...
    def test_pages_read_numeric_cloud_and_server_display_urls(self) -> None:
        page = {
            "id": "9",
            "title": "Cache plan",
            "body": {"storage": {"value": "<h1>Plan</h1><p>Use &amp; reuse.</p>"}},
        }
        for cloud, url, response in [
            (True, "https://team.atlassian.net/wiki/spaces/DEV/pages/9/Cache", page),
            (
                False,
                "https://work.example/confluence/display/DEV/Cache+plan",
                {"results": [page]},
            ),
        ]:
            with self.subTest(cloud=cloud):
                ...
                if cloud:
                    self.assertEqual(
                        fake.calls[0][1], "rest/api/content/9?expand=body.storage"
                    )
                else:
    ...
    def test_external_and_context_escape_urls_never_reach_transport(self) -> None:
        ...
        providers = Providers(config(False), fake)
        for url in [
            "https://evil.example/bitbucket/projects/TEAM/repos/repo/pull-requests/8",
            "https://work.example/other/projects/TEAM/repos/repo/pull-requests/8",
            "https://work.example/bitbucket/../projects/TEAM/repos/repo/pull-requests/8",
            "https://work.example/bitbucket/%252e%252e/projects/TEAM/repos/repo/pull-requests/8",
        ]:
    ...
    def test_pr_creation_reuses_exact_branch_pair_on_second_cloud_page(self) -> None:
        fake = FakeTransport(
            [
                {
                    "values": [cloud_pr("other")],
                    "next": "https://api.bitbucket.org/2.0/repositories/team/repo/pullrequests?page=2",
                },
                {"values": [cloud_pr()]},
            ]
        )
        result = Providers(config(), fake).create_pull_request(
            "team/repo", "feature", "main", "Cache", "Details"
        )
            ...
            with self.subTest(cloud=cloud):
                fake = FakeTransport(
                    [
                        ...
                        cloud_pr() if cloud else server_pr(),
                    ]
                )
                Providers(config(cloud), fake).create_pull_request(
                    "team/repo" if cloud else "TEAM/repo",
                    "feature",
                    "main",
                    "Cache",
                    "Details",
                ...
                if cloud:
                    ...
                    self.assertEqual(
                        payload["toRef"]["repository"]["project"]["key"], "TEAM"
                    )
        ...
        for cloud in (True, False):
            for draft in (True, False, None):
                with self.subTest(cloud=cloud, draft=draft):
                    ...
                    result = Providers(config(cloud), fake).create_pull_request(
                        "team/repo" if cloud else "TEAM/repo",
                        "feature",
                        "main",
                        "Cache",
        ...
        for source, target in (("wrong-source", "main"), ("feature", "wrong-target")):
            with self.subTest(source=source, target=target):
                ...
                with self.assertRaises(ProviderError) as failure:
                    Providers(config(), fake).create_pull_request(
                        "team/repo", "feature", "main", "Cache", "Details"
                    )
    ...
    def test_server_pr_pagination_uses_next_page_start(self) -> None:
        fake = FakeTransport(
            [
                {
                    "values": [server_pr("other")],
                    "isLastPage": False,
                    "nextPageStart": 47,
                },
                {"values": [server_pr()], "isLastPage": True},
            ]
        )
        Providers(config(False), fake).create_pull_request(
            "TEAM/repo", "feature", "main", "Cache", "Details"
        )
    ...
    def test_server_project_case_does_not_create_a_duplicate_pr(self) -> None:
        ...
        result = Providers(config(False), fake).create_pull_request(
            "team/repo", "feature", "main", "Cache", "Details"
        )
    ...
    def test_comment_search_paginates_before_sending(self) -> None:
        fake = FakeTransport(
            [
                {"comments": [{"id": "1", "body": "Old"}], "startAt": 0, "total": 2},
                {
                    "comments": [
                        {"id": "2", "body": "Already delivered\n[masteragent:task-1]"}
                    ],
                    "startAt": 1,
                    "total": 2,
                },
        ...
        result = Providers(config(False), fake).comment_issue(
            "APP-12", "Update", "task-1"
        )
    ...
    def test_cross_origin_pagination_rejected_before_next_request(self) -> None:
        fake = FakeTransport(
            [{"values": [], "next": "https://evil.example/pullrequests"}]
        )
    ...
    def test_credentials_resolve_once_for_only_used_provider(self) -> None:
        settings = config()
        settings["jira"].update(
            token_env="TEST_JIRA_TOKEN", username_env="TEST_JIRA_EMAIL"
        )
        clients: list[tuple] = []
        ...
        with patch.dict(
            os.environ,
            {"TEST_JIRA_TOKEN": "private-token", "TEST_JIRA_EMAIL": "rory@example.com"},
            clear=True,
        ):
    ...
    def test_uncertain_write_failure_is_not_replayed(self) -> None:
        fake = FakeTransport(
            [
                {"comments": [], "total": 0},
                ProviderError("Lost response", uncertain=True),
            ]
        )
        with self.assertRaises(ProviderError) as failure:
...
class TransportTests(unittest.TestCase):
    def test_context_prefix_and_headers_are_preserved(self) -> None:
        ...
        client = HttpTransport(
            "https://work.example/jira", "Bearer private-token", opener=opener
        )
        self.assertTrue(client.request("GET", "rest/api/2/issue/APP-12")["ok"])
        self.assertEqual(
            opener.calls[0].full_url,
            "https://work.example/jira/rest/api/2/issue/APP-12",
        )
        self.assertEqual(
            opener.calls[0].get_header("Authorization"), "Bearer private-token"
        )
    ...
    def test_read_retry_is_bounded_and_retry_after_capped(self) -> None:
        ...
        errors = [
            HTTPError(
                "https://work.example/jira",
                429,
                "token should not appear",
                {"Retry-After": "999"},
                io.BytesIO(b"private content"),
            )
            for _ in range(3)
        ]
    ...
    def test_http_503_write_is_uncertain_and_not_retried(self) -> None:
        ...
        client = HttpTransport(
            "https://work.example/jira", "Bearer token", opener=opener
        )
        with self.assertRaises(ProviderError) as failure:
            client.request("POST", "rest/api/2/issue/APP-12/comment", {"body": "Done"})
        self.assertTrue(failure.exception.uncertain)
    ...
    def test_redirect_blocks_cross_origin_and_context_before_auth_can_leave(
        self,
    ) -> None:
        ...
        for target in [
            "https://evil.example/jira",
            "https://work.example/login",
            "http://work.example/jira",
        ]:
        ...
        allowed = redirect.redirect_request(
            ...
            "Found",
            {},
            "https://work.example/jira/rest/api/2/issue/APP-13",
        )
    ...
    def test_write_redirect_is_not_followed(self) -> None:
        redirect = _ScopedRedirect("https://work.example/jira")
        request = Request(
            "https://work.example/jira/comment", method="POST", data=b"{}"
        )
        with self.assertRaises(ProviderError) as failure:
            redirect.redirect_request(
                request, None, 302, "Found", {}, "https://work.example/jira/other"
            )
        self.assertTrue(failure.exception.uncertain)

    def test_url_credentials_and_encoded_traversal_rejected(self) -> None:
        for url in [
            "https://user:secret@work.example/jira/rest/api",
            "https://work.example/jira/%2e%2e/other",
            "https://work.example/jira/%252e%252e/other",
            "https://work.example/jira%2fother",
            "https://work.example/jira/../other",
            "https://work.example/jira\\other",
            "https://work.example/jira-other",
        ]:
            with self.subTest(url=url), self.assertRaises(ProviderError):
