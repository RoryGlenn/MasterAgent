...
SHOULD be reused within a workflow. Repository subprocess environments MUST
exclude configured provider credential variables and conventional provider
TOKEN/USERNAME variables while preserving unrelated build and Git settings.
Parent provider clients MUST retain access to their credentials. The host assistant MUST own interpretation,
...
### Repository subprocesses do not inherit provider credentials

- GIVEN provider credentials are present in the parent environment
- WHEN Simple runs project checks or Git operations with repository hooks
- THEN child environments omit those credentials and retain ordinary settings
- AND parent provider requests can still use the credentials afterward
...
- Modified by GitHub issue #186 to make the subprocess credential boundary explicit.
