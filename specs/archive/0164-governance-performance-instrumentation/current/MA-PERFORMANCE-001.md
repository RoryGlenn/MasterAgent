...
Until issue #170 supplies a trusted configuration-owned implementation identity,
every selected implementation MUST be represented exactly as
`unbound_pending_170` with `bound = false`. #164 MUST NOT claim `native`, exact
implementation binding, or drift validation.
...
unbound #170 placeholders; record zero governance-induced and approval
