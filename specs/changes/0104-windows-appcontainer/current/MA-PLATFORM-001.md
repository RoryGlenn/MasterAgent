...
Windows MAY advertise `capsule_isolation` only when the zero-capability
AppContainer worker-launch backend, explicit handle selection, private runtime
projection, and Job Object limits are all available. The backend identity and
security-relevant runtime component digests MUST be stable and secret-free.
...
native Windows areas without equivalent implementation and evidence MUST
remain planned and unavailable independently.
...
- GIVEN a Linux, macOS, or native Windows host and a capability-capsule worker
  request
...
- AND Windows reports the AppContainer backend only after its native symbol,
  identity, ACL, handle, Job Object, and standard-user evidence is complete
...
- Added native Windows AppContainer isolation in GitHub issue #104.
