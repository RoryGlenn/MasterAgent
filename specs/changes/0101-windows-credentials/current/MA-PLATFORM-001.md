...
contracts. Platform-neutral package and CLI imports MUST NOT load operating-
system-specific modules until an operation selects the relevant backend.
Selection MUST expose stable, secret-free backend identity and availability,
MUST preserve certified POSIX behavior, and MUST NOT substitute a compatibility
shim or weaker fallback when an equivalent secure backend is unavailable.

The `credential_storage` contract MUST represent a hardened native secret
provider rather than the process environment or restricted-file development
adapter. Native Windows MAY advertise it only after its Credential Manager,
current-user DPAPI, secure-filesystem, locking, and atomic-publication probes
succeed. Other platforms MUST report it unavailable until an equivalent native
provider is certified, without blocking their existing environment or
restricted-file adapters.
...
selected with `require_os_sandbox=False` MUST NOT advertise production
isolation.
...
state, credentials, connector construction, provider access, or effects. Each
separate native Windows backend and hosted-certification area MUST remain
planned until it supplies equivalent implementation and evidence. Progressive
doctor MAY report the bounded absent-profile setup state without opening
profile bytes. Reading an existing or explicit profile MUST require
...
credential, process, Git, or capsule guarantee it requires. Explicit identities
prevent a future port from silently weakening that guarantee.

## Scenarios

### Windows configuration diagnostics stay usable

- GIVEN MasterAgent is installed on Windows before every native secure backend
  is implemented
...
### Native credential status remains truthful

- GIVEN a platform has environment or restricted-file credential adapters but
  no certified native secret provider
- WHEN readiness reports `credential_storage`
- THEN the contract remains unavailable without blocking those legacy adapters
- AND native Windows reports it available only after Credential Manager and
  current-user DPAPI probes succeed
...
- Clarified trusted Linux executable availability in GitHub issue #126.
- Added the native credential-storage contract in GitHub issue #101.
